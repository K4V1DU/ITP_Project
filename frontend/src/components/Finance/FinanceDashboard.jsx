  ...
  return (
    <Container maxWidth="xl">
      <Box sx={{
        ...
        {/* Header */}
        <Paper elevation={0} sx={{ p: 3, mb: 4, background: iceColors.gradient3, borderRadius: 4 }}>
          <Box display="flex" alignItems="center" justifyContent="space-between">
            ...
            <Tooltip title="Refresh Data">
              <IconButton onClick={fetchPayments} sx={{ color: 'white' }} disabled={loading}>
                <RefreshIcon />
              </IconButton>
            </Tooltip>
          </Box>
        ...
        <Grid container spacing={3} mb={4}>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard label="Approved Payments" value={approved} gradient={statusConfigs.Approved.gradient} icon={<VerifiedIcon sx={{ fontSize: 40 }} />} />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard label="Pending Review" value={pending} gradient={statusConfigs.Pending.gradient} icon={<PendingActionsIcon sx={{ fontSize: 40 }} />} />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard label="Rejected Payments" value={rejected} gradient={statusConfigs.Rejected.gradient} icon={<BlockIcon sx={{ fontSize: 40 }} />} />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard label="Total Orders" value={total} gradient={iceColors.gradient4} icon={<AttachMoneyIcon sx={{ fontSize: 40 }} />} />
          </Grid>
        </Grid>
            ...
            <Grid container spacing={2} alignItems="center">
              <Grid item xs={12} md={3}>
                <FormControl fullWidth size="small">
                  <InputLabel><FilterListIcon sx={{ mr: 1 }} />Filter by Status</InputLabel>
              ...
              <Grid item xs={12} md={6}>
                <TextField
                  fullWidth size="small" placeholder="Search by Order # or Notes..."
              ...
              <Grid item xs={12} md={3}>
                <Box display="flex" gap={1} justifyContent="flex-end">
                  <Badge badgeContent={filtered.length} color="primary">
                    <Chip label={`${filtered.length} Results`} color="primary" variant="outlined" />
                  </Badge>
                </Box>
              </Grid>
            </Grid>
        ...
        <Grow in timeout={1000}>
          <Card sx={{ borderRadius: 4, boxShadow: '0 10px 40px rgba(0,0,0,0.1)', overflow: 'hidden', background: 'white' }}>
                ...
                {loading ? (
                  ...
                  currentData.map((payment, index) => (
                    <TableRow
                      ...
                      hover
                      sx={{ '&:hover': { background: iceColors.vanilla, transform: 'translateX(5px)', transition: 'all 0.3s' } }}
                    >
                      <TableCell>
                              ...
                              <Tooltip title="Approve Payment">
                                <IconButton
                                  ...
                                  sx={{
                                    ...
                                    '&:hover': { bgcolor: statusConfigs.Approved.color, color: 'white', transform: 'scale(1.05)' }
                                  }}
                                >
                                  <VerifiedIcon />
                                </IconButton>
                              ...
                              <Tooltip title="Reject Payment">
                                <IconButton
                                  ...
                                  sx={{
                                    ...
                                    '&:hover': { bgcolor: statusConfigs.Rejected.color, color: 'white', transform: 'scale(1.05)' }
                                  }}
                                >
                                  <BlockIcon />
                                </IconButton>
            ...
            <TablePagination
              ...
              sx={{ background: iceColors.vanilla, borderTop: '2px solid #e0e0e0' }}
            />
          </Card>
        </Grow>
        ...
        <Grid container spacing={3} mt={4}>
          <Grid item xs={12} md={6}>
            <Fade in timeout={1200}>
              <Card sx={{ p: 3, borderRadius: 4, background: 'white', boxShadow: '0 10px 40px rgba(0,0,0,0.1)' }}>
          ...
          <Grid item xs={12} md={6}>
            <Fade in timeout={1200}>
              <Card sx={{ p: 3, borderRadius: 4, background: `linear-gradient(135deg, ${iceColors.mint} 0%, ${iceColors.vanilla} 100%)`, boxShadow: '0 10px 40px rgba(0,0,0,0.1)' }}>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={6}>
                    <Paper sx={{ p: 2, textAlign: 'center', background: 'white' }}>
                      ...
                      <Typography variant="body2" color="textSecondary">Rejection Rate</Typography>
                    </Paper>
                  </Grid>
                  <Grid item xs={12}>
                    <Paper sx={{ p: 2, textAlign: 'center', background: 'white' }}>
                      <PendingActionsIcon sx={{ fontSize: 40, color: statusConfigs.Pending.color }} />
                      <Typography variant="h4" fontWeight="bold" color={statusConfigs.Pending.color}>{pending}</Typography>
        ...
        {/* Confirmation Dialog */}
        <Dialog open={confirmOpen} onClose={() => !processing && setConfirmOpen(false)} PaperProps={{ sx: { borderRadius: 3, minWidth: 400 } }}>
        ...
        <Dialog open={deleteConfirmOpen} onClose={() => !processing && setDeleteConfirmOpen(false)} PaperProps={{ sx: { borderRadius: 3, minWidth: 400 } }}>
